...
import { ErrorCategories, getErrorDetails, createErrorWithCode } from '../utils/errorCodes.js';
 ...
 */
export const createError = (message, statusCode = 500, type = ErrorTypes.INTERNAL) => {
...
export const errorHandler = (err, req, res, next) => {
  ...
  const statusCode = err.statusCode || (errorDetails && errorDetails.statusCode) || 
                    (res.statusCode === 200 ? 500 : res.statusCode);
  
  const errorType = err.type || (errorDetails && errorDetails.category) || getDefaultErrorType(statusCode);
  
  // Create error response
  const errorResponse = {
    success: false,
    error: {
      message: err.message || (errorDetails && errorDetails.message) || 'Something went wrong',
      type: errorType,
