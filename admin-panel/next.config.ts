      ...
      if (config.optimization.minimizer) {
        ...
        config.optimization.minimizer.forEach((minimizer: any) => {
          if (minimizer.constructor && minimizer.constructor.name === 'TerserPlugin') {
            if (minimizer.options && minimizer.options.terserOptions && minimizer.options.terserOptions.compress) {
              minimizer.options.terserOptions.compress.drop_console = false; // Keep console for debugging
              minimizer.options.terserOptions.compress.pure_funcs = ['console.debug']; // But remove console.debug
            }
          }
        });
      }
