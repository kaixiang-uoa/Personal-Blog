...
const createIndex = async (model, index, options = {}) => {
  try {
    ...
    logger.info(
      `✅ Created index on ${model.collection.name}: ${JSON.stringify(index)}`,
    );
  } catch (error) {
    if (error.code === 85) {
      // Index already exists
      logger.info(
        `ℹ️ Index already exists on ${model.collection.name}: ${JSON.stringify(index)}`,
      );
    } else {
      logger.error(
        `❌ Error creating index on ${model.collection.name}: ${error.message}`,
      );
    }
  }
};
...
const ensureIndexes = async () => {
  try {
    // Connect to MongoDB
    const dbUri =
      process.env.MONGODB_URI || 'mongodb://localhost:27017/personal-blog';
